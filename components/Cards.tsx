...
export const FeaturedCard = ({item : { image, rating, name, address, price}, onPress}: Props) => {
    return (
        <TouchableOpacity onPress={onPress} className='flex flex-col items-start w-60 h-80 relative'>
            ...
            <View className='flex flex-row items-center bg-white/90 px-3 py-1.5 rounded-3xl absolute top-5 right-5'>
...
export const Card = ({item : { image, rating, name, address, price}, onPress}: Props) => {
    return (
        <TouchableOpacity onPress={onPress} className='flex-1 w-full mt-4 px-3 py-4 rounded-lg bg-white shadow-lg shadow-black-100/70 relative'>
            <View className='flex flex-row items-center bg-white/90 px-3 py-1.5  absolute top-5 right-5  p-1 rounded-full z-50'>
            ...
            <View className='flex flex-col mt-2'>
                ...
                <View className='flex flex-row justify-between mt-2 items-center w-full'>
                    <Text className='text-base font-rubik-bold text-primary-300 '>
                        $ {price}
                    </Text>
                    <Image source={icons.heart} className='w-5 h-5 mr-2 size-5' tintColor='#000000'/>
